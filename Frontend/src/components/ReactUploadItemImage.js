...
class ReactUploadItemImage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      if (selectedfile) {
        ...
        if (isVerified) {
          if (
            ...
            alert(
              "You are again changing the item image. Click upload if you actually want to do it"
            );
          }
        }
      }
  ...
  fileUploadHandler = () => {
    console.log("in file upload handler", this.props.itemId);
    const formData = new FormData();

    console.log(JSON.stringify(this.props.itemStore.item));
    console.log("&&&&&&&&", +this.props.itemStore.item.item_Id);
    formData.append("myImage", this.state.file);
    formData.append("itemID", this.props.itemStore.item.item_Id);
    ...
    axios
      .post("http://localhost:3001/grubhub/item/upload", formData, config)
      .then(response => {
        alert("The file is successfully uploaded");

        this.setState({ redirect: <Redirect to="/OwnerHome" /> });
      });
  };
...
export default connect(mapStateToProps)(ReactUploadItemImage);
