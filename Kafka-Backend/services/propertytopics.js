...
var Properties = require("../models/CustomersSchema");
var Bookings = require("../models/CustomersSchema");
...
function propertylistings(msg, callback) {
  ...
  if (msg.body.fromdate && msg.body.todate) {
    query["$and"].push(
      { startDate: { $lte: msg.body.fromdate } },
      { endDate: { $gte: msg.body.todate } }
    );
  }
  ...
  Properties.find(query)
    ...
    .exec(function(error, result) {
      if (error) {
        ...
        async.eachOfSeries(
          result,
          function(value, i, inner_callback) {
            value.bookedFrom = [];
            value.bookedTo = [];
            value.bookedBy = [];
            value.noOfGuests = [];
              ...
              if (!error) {
                if (bookings) {
                  bookings.forEach(function(bookingResult) {
                    ...
                    var tempbookedFrom =
                      bookingResult.bookedFrom.getFullYear() +
                      "-" +
                      (bookingResult.bookedFrom.getMonth() + 1) +
                      "-" +
                      bookingResult.bookedFrom.getDate();
                    value.bookedFrom.push(tempbookedFrom);
                    var tempbookedTo =
                      bookingResult.bookedTo.getFullYear() +
                      "-" +
                      (bookingResult.bookedTo.getMonth() + 1) +
                      "-" +
                      bookingResult.bookedTo.getDate();
  ...
  Bookings.find(
    {
      $or: [
        {
          $and: [
            { bookedFrom: { $gte: new Date(msg.body.startDate) } },
            { bookedTo: { $lte: new Date(msg.body.endDate) } }
          ]
        },
        {
          $and: [
            { bookedTo: { $gte: new Date(msg.body.startDate) } },
            { bookedTo: { $lte: new Date(msg.body.endDate) } }
          ]
        },
        {
          $and: [
            { bookedFrom: { $gte: new Date(msg.body.startDate) } },
            { bookedFrom: { $lte: new Date(msg.body.endDate) } }
          ]
        },
        {
          $and: [
            { bookedFrom: { $lte: new Date(msg.body.startDate) } },
            { bookedTo: { $gte: new Date(msg.body.endDate) } }
          ]
        }
      ]
    },
...
function triplistings(msg, callback) {
  ...
  if (msg.body.fromdate && msg.body.todate) {
    query["$and"].push(
      { bookedFrom: { $lte: msg.body.fromdate } },
      { bookedTo: { $gte: msg.body.todate } }
    );
  }
      ...
      if (error) {
        ...
        async.eachOfSeries(
          ...
          function(value, i, inner_callback) {
            ...
            Properties.find({ _id: value.propertyID }, function(
              error,
              property
            ) {
              if (!error) {
