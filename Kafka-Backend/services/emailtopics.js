var Emails = require("../models/CustomersSchema");
...
function sendmail(msg, callback) {
  ...
  Emails.findOneAndUpdate({ _id: msg.id }, { $set: msg.reply }, function(
    error,
    response
  ) {
    if (error) {
...
function getsentemails(msg, callback) {
  ...
  Emails.find({ senderEmailAddress: msg.senderEmailAddress }, function(
    error,
    result
  ) {
    if (error) {
